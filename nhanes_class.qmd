...
```{r}
demos <- select(demographics, SEQN, RIDAGEYR)
```

### Merge Subsets Into A Working Dataframe as 'df'

```{r}
df <- list(diet, tiid, bhg, uhg, demos)
...
```{r}
joined_data <- read_csv("data/nhanes_data/all_data_joined.csv")
```

2.  Do some exploratory data analysis

```{r}
ggplot(df, aes(x = DIQ010, fill = DRD370B)) + geom_bar()
...
```{r}
df <- df %>%
  mutate(over40 = ifelse(RIDAGEYR > 40, "old", "not_as_old"))

tab <- df %>%
  tabyl(RIDAGEYR) %>%
  adorn_percentages("col") %>%   # percent of total
  adorn_pct_formatting(digits = 1)

percentage_over_age_of_40 <- 41.1 
```
...
likelihood_over40_has_diabetes <- .95
```

```{r}
(likelihood_over40_has_diabetes * prior_probability_has_diabetes)/ percentage_over_age_of_40
```
