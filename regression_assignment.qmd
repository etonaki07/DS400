...
ggplot(penguins_complete, aes(x = flipper_length_mm, y = body_mass_g)) +
  ...
  stat_cor(method = "pearson", 
           ...
           label.x = 180, label.y = 6300, size = 5) +
  labs(
...
1.  What is the correlation between flipper length and body mass?
    Flipper length and body mass show a strong positive correlation, meaning penguins with longer flippers tend to weigh more. This shows a clear linear relationship between overall size and flipper length.
2.  Which species tends to be heaviest? Lightest?
    Gentoo is the heaviest, then Chinstrap, and Adelie are the lightest.
3.  Does the relationship between flipper length and body mass appear consistent across species?
    All species show the same general trend where larger flipper length corresponds to higher body mass, but the strength and position of the relationship differ by species.
4.  Which predictors do you think will be most useful?
    Flipper length and species would be the strongest predictors because they show the clearest structure in the data.
:::
...
penguin_model_1 <- stan_glm(
  ...
  prior = normal(location = 50, scale = 25),

  prior_aux = exponential(rate = 1/500),
  chains = 4,
  iter = 10000,       
  seed = 84735
)
...
Look for: - Is the slope positive (heavier penguins have longer flippers)? The slope is positive. 
- What's the typical prediction error (sigma)? 
:::
...
penguin_model_2 <- stan_glm(
  ...
  prior = normal(location = 0, scale = 1000),
  prior_aux = exponential(rate = 1/500),
  chains = 4,
  iter = 10000,
  seed = 84735
...
pp_check(penguin_model_1) +
  ggtitle("Posterior Predictive Check: Model 1 (Flipper Length Only)")
```
...
pp_check(penguin_model_2) +
  ggtitle("Posterior Predictive Check: Model 2 (Species Only)")
```
...
pp_check(penguin_model_3) +
  ggtitle("Posterior Predictive Check: Model 3 (Flipper Length + Species)")
```
...
pp_check(penguin_model_4) +
  ggtitle("Posterior Predictive Check: Model 4 (Physical Measurements Only)")
...
cv_model_1 <- prediction_summary_cv(
  model = penguin_model_1,
  data = penguins_complete,
  k = 10
)
cv_model_1
...
cv_model_2 <- prediction_summary_cv(
...
cv_model_3 <- prediction_summary_cv(
...
cv_model_4 <- prediction_summary_cv(
...
|-------|-------------------|-------------|----------------|
| 1     | flipper only      | 269.2779    | Flipper length alone predicts body mass reasonably |                                            well; captures general size trends.                 |
| 2     | species only      | 338.2979	  | Species alone is less accurate; captures differences                                             between species but misses variation within species.|
| 3     | flipper + species | 252.7179    | Best predictive accuracy; combines flipper size and                                              species differences for improved prediction.|
| 4     | all measurements  | 271.7703    | Physical measurements beyond flipper length do not                                               substantially improve accuracy over Model 3; slightly                                             higher MAE.|
...
Model 3 provides the best combination of predictive power and simplicity without unnecessary complexity
:::
...
Model 3, using flipper length and species, performed the best in cross-validation with the lowest MAE. It wasn’t the most complex model since Model 4 added bill measurements but didn’t improve predictions much.
2.  **Species vs Measurements**: Does knowing the species add information beyond physical measurements? How can you tell?
Knowing the species does add some extra information beyond flipper length because Model 3 predicts slightly better than Model 1, but adding bill measurements in Model 4 didn’t make a big difference.
3.  **Practical Use**: If you were a penguin researcher with limited measurement tools, which model would you use and why?
If I only had basic tools, I’d go with Model 1 using just flipper length because it’s easy to measure and still does a pretty good job predicting body mass.
4.  **Prior Sensitivity**: How might different priors have affected your results? Were your priors informative or vague?
Different priors could have shifted the results a little, but the priors we used were reasonable and wide enough that the data mostly drove the estimates.
5.  **Assumptions**: What assumptions does normal regression make? Are they reasonable for penguin body mass?
Normal regression assumes linear relationships, normally distributed residuals, consistent variance, and independent observations. These seem pretty reasonable for penguin body mass given the patterns in the data.
:::
...
fake_news %>%
  ...
  geom_bar() +
  labs(title = "Real vs Fake Articles",
       x = "News Type",
...
fake_news %>%
  ...
  geom_boxplot() +
  labs(title = "Title Length (Number of Words) by News Type",
       x = "News Type",
...
fake_news %>%
  ...
  geom_boxplot() +
  labs(title = "Negative Sentiment by News Type",
       x = "News Type",
...
fake_news %>%
mutate(
    title_excl = ifelse(is.na(title_excl), 0, title_excl),
    text_excl  = ifelse(is.na(text_excl), 0, text_excl),
    has_excl   = ifelse(title_excl > 0 | text_excl > 0, 1, 0)
  ) %>%
  filter(has_excl == 1) %>%
  count(type) %>%
...
fake_news %>%
  mutate(has_excl = ifelse(title_excl > 0 | text_excl > 0, 1, 0)) %>%
  summarise(total_with_excl = sum(has_excl))
...
ci_logodds <- posterior_interval(fake_news_model, prob = 0.80)
ci_or <- exp(ci_logodds)
ci_or
```
...
1.  **title_words** (OR = 0.83 to 0.96): How does title length relate to fake vs real news?
    Articles with exclamation points have 79–97% lower odds of being REAL news (80% CI: 0.03–0.21). This means exclamation points are strongly associated with FAKE news.
2.  **negative** (OR = 0.60 to 0.88): How does negative sentiment relate to fake vs real news?
  Higher negative sentiment scores reduce the odds of being REAL news by 12–40% (80% CI: 0.60–0.88), meaning articles with more negative language are more likely to be FAKE news.
**Summary question:** Based on these odds ratios, what characteristics define fake news in this dataset?
  In summary, fake news articles in this dataset are more likely to have exclamation points, longer titles, and higher negative sentiment.
:::
