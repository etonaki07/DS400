...
```{r}
ggplot(weather_WU, aes(x = location, y = temp3pm, fill = location)) +
  geom_boxplot()
```

```{r}
ggplot(weather_WU, aes(x = temp3pm, fill = location)) + geom_histogram()
```

```{r}
ggplot(weather_WU, aes(x = humidity9am, y = temp9am, color = location)) + geom_point() + geom_smooth()
```
